...
class JournalController {
    ...
    public async getSummary(req: Request, res: Response) {
        try {
            ...
            const journals = await journalService.getSummary(userId, new Date(startDate as string), new Date(endDate as string));
            res.status(200).json({ success: true, message: "Summary", data: journals });
        } catch (error) {
